...
export default function WalletCalendar({
  onDateSelect,
  initialDate,
  ...
  // Modified: Only trigger onDateSelect on mount if auto-selection is not disabled
  useEffect(() => {
    if (!hasInitialized && onDateSelect && !disableAutoSelection) {
      setHasInitialized(true);
      if (!initialDate) onDateSelect(defaultDate);
    } else {
      // Just mark as initialized but don't trigger date selection
      setHasInitialized(true);
    }
  }, [
    onDateSelect,
  ...
  const handleDateSelect = (date: Date, e: React.MouseEvent) => {
    ...
    if (
      newSelectedDate.getMonth() !== currentMonth ||
